...
void si5351aSetFrequency(float frequency) {
    ...
    divider = 800000000 / frequency;  // Calculate the division ratio.
                                      // 900,000,000 is the maximum internal
    // PLL frequency: 800MHz
    if (divider % 2) divider--;  // Ensure an even integer division ratio

    pllFreq = divider * frequency;  // Calculate the pllFrequency: the divider *
                                    // desired output frequency

    mult = pllFreq / xtalFreq;  // Determine the multiplier to get to the
                                // required pllFrequency
    ...
    // reprented by constants SI_R_DIV1 to SI_R_DIV128 (see si5351a.h header
    // file) If you want to output frequencies below 1MHz, you have to use the
...
void si5351aInitialize(void) {
    system("raspi-gpio set 13 op pd");
    system("raspi-gpio set 13 dl");
    gpioInitialise();
    signal(SIGINT, si5351aCleanup);
}

void si5351aRelayOn(void) { system("raspi-gpio set 13 dh"); }

void si5351aRelayOff(void) { system("raspi-gpio set 13 dl"); }
